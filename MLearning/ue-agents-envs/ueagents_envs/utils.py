...
from ueagents_envs.logging_util import get_logger, DEBUG
...
def validate_environment_path(env_path: str) -> Optional[str]:
    ...
    if get_platform() == "linux" or get_platform() == "linux2":
        ...
        if len(candidates) == 0:
            # Look for e.g. 3DBall\UnrealEnvironment.exe
            crash_handlers = set(
...
def launch_executable(file_name: str, args: List[str]) -> subprocess.Popen:
    """
    Launches a Unreal executable and returns the process handle for it.
