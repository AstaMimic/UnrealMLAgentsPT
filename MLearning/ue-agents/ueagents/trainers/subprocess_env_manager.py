...
from ueagents.trainers.env_manager import EnvManager, EnvironmentStep, AllStepResult
from ueagents.trainers.settings import TrainerSettings
from ueagents_envs.timers import (
    TimerNode,
    timed,
    hierarchical_timer,
    reset_timers,
...
from ueagents.trainers.settings import ParameterRandomizationSettings, RunOptions
from ueagents.trainers.action_info import ActionInfo
from ueagents_envs.side_channel.environment_parameters_channel import (
    EnvironmentParametersChannel,
)
from ueagents_envs.side_channel.stats_side_channel import (
    EnvironmentStats,
    StatsSideChannel,
)
from ueagents.trainers.training_analytics_side_channel import (
...
class UnityEnvWorker:
    ...
    def send(self, cmd: EnvironmentCommand, payload: Any = None) -> None:
        try:
            ...
            raise UnrealCommunicationException("UnrealEnvironment worker: send failed.")
    ...
    def recv(self) -> EnvironmentResponse:
        try:
            ...
            raise UnrealCommunicationException("UnrealEnvironment worker: recv failed.")
...
def worker(
    ...
    env_factory: Callable[
        [int, List[SideChannel]], UnrealEnvironment
    ...
    try:
        ...
        logger.debug(f"UnrealEnvironment worker {worker_id}: environment stopping.")
        step_queue.put(
        ...
        logger.exception(
            f"UnrealEnvironment worker {worker_id}: environment raised an unexpected exception."
        )
        step_queue.put(
            EnvironmentResponse(EnvironmentCommand.ENV_EXITED, worker_id, ex)
        )
        ...
        logger.debug(f"UnrealEnvironment worker {worker_id} closing.")
        if env is not None:
            env.close()
        logger.debug(f"UnrealEnvironment worker {worker_id} done.")
        parent_conn.close()
...
class SubprocessEnvManager(EnvManager):
    ...
    def _assert_worker_can_restart(self, worker_id: int, exception: Exception) -> None:
        ...
        If the restart limit is exceeded it will raise a UnrealCommunicationException.
        If the exception is not recoverable it re-raises the exception.
        """
        if (
            isinstance(exception, UnrealCommunicationException)
            or isinstance(exception, UnrealTimeOutException)
            or isinstance(exception, UnrealEnvironmentException)
            or isinstance(exception, UnrealCommunicatorStoppedException)
        ):
