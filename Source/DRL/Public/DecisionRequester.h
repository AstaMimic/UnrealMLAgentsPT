 ...
 *
 * The UDecisionRequester component is responsible for managing how frequently an agent requests decisions and performs actions
 * in a reinforcement learning environment. It hooks into the simulation's stepping mechanism and ensures that agents request decisions
...
protected:
	 ...
	 *
	 * This function initializes the component and subscribes to the relevant events in the simulation, such as the Academy's pre-step event.
	 */
...
public:
	 ...
	 *
	 * By setting this value, the timing of decision requests can be staggered among agents with the same `DecisionPeriod`.
	 * This value can range from 0 to `DecisionPeriod - 1`.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "DecisionRequester", meta = (ClampMin = "0", ClampMax = "19"))
	int DecisionStep = 0;

	/** @brief Indicates whether the agent will take actions during the Academy steps where it does not request a decision.
	 *
	 * If this value is `true`, the agent will continue to take actions between decisions, even if it does not request a decision.
	 */
...
private:
	 ...
	 *
	 * The `UAgent` object to which this `DecisionRequester` component is attached, controlling its decision-making process.
	 */
	UPROPERTY()
	UAgent* Agent;
	 ...
	 *
	 * This function is called before every Academy step to determine whether the agent should request a decision or take an action.
	 *
	 ...
	 *
	 * This function handles the logic for whether an agent should request a decision or action, based on the configured `DecisionPeriod` and `DecisionStep`.
	 *
	 * @param AcademyStepCount The current step count of the Academy.
	 */
	 ...
	 *
	 * This function checks if the current step count matches the configured `DecisionPeriod` and `DecisionStep` to decide if a decision should be requested.
	 *
	 * @param AcademyStepCount The current step count of the Academy.
